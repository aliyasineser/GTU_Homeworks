...
/* Written by Ali Yasin Eser, on 10 may 2016*/
/* Student ID:141044058 for Homework 6 */
...
#define MSG_QUEUE_STR_SIZE 32768
...
void *grepfromFileThread(void *args) {
    ...
    grepfromfile(data->file_name, data->key,data->parent, data->msgQueueId);
    pthread_exit(NULL);
...
int msgwrite(void *buf, int len, int queueId);
int grepfromfile(const char* file_name, const char* key, const pid_t parent, int msgQueueId);
...
int grepfromdirectory(const char* directory_name, const char* key, int fifoDesc, pid_t parent) {
	...
	int size;
	int msgQueueKey;
	char tempPathForGettingCurrentPath[PATH_MAX];
	mymsg_t *stringForSum;
	int strSize = MSG_QUEUE_STR_SIZE ;
	...
	if ((msgQueueKey = ftok(tempPathForGettingCurrentPath, 1 ))== (key_t)-1)
		perror("Failed to derive key from current directory");	


	if ((msqid = msgget(msgQueueKey , MSG_PERM | IPC_CREAT)) == -1)
    ...
    while ((direntp = readdir(dirp)) != NULL) {
        ...
        if (stat(currentDirectoryEntryPath, &statbuf) == -1)
			...
			threadDataArrayPtr[countForForkLoop].msgQueueId = msqid;
    ...
    while(TRUE){
  	  stringForSum = (mymsg_t *)malloc(sizeof(mymsg_t) + strSize - 1);
  	  strcpy(stringForSum->mtext," ");
  	  /*fprintf(stderr, "%ld lel\n", (long)getpid());*/
    	size = msgrcv(  msqid, stringForSum, strSize, 1, IPC_NOWAIT );
    	...
    	if(size < 0){
    		if(errno  == E2BIG){
    			...
    			perror("There is an error, couldn't read the message from the message queue: ");
    			exit(1);
    		}
    	}else{
    	/*fprintf(stderr, "%s\n", stringForSum->mtext);*/
		r_write(fifoDesc, stringForSum->mtext, strlen(stringForSum->mtext));
...
int grepfromfile(const char* file_name, const char* key,  const pid_t parent, int msgQueueId) {
	...
	/*Temp file, we will fill this file with founds and read from there, after that we will remove it*/
	sprintf(temp,"%s_tempFile.txt", file_name);
	
	if ((tempFileForSumFd = open(temp, O_WRONLY | O_CREAT | O_TRUNC, WRITE_PERMS)) == -1 ){
    ...
    stringOfFullFile = (char*) calloc(count+1,sizeof (char) );
    ...
    write(tempFileForSumFd, pathOfTheFile, strlen(pathOfTheFile));

	/*No character found in file, empty file*/
	if(count == 0){
		sprintf(stringForWritingToFile, "\tEmpty file\n");
		write(tempFileForSumFd, stringForWritingToFile, strlen(stringForWritingToFile));
    ...
    stringOfFullFile = (char*) calloc((count+1),sizeof (char));
	stringOfFullFile[0] = '\0';
    r_read(iptr, stringOfFullFile, count);
    
	
    /*fprintf(stderr, "%s\n", stringOfFullFile);*/
    /*fprintf(stderr, "msg id -____-> %d\nParent id -> %ld\n\n", msgQueueId, (long)parent);*/
    msgwrite(stringOfFullFile, strlen(stringOfFullFile), msgQueueId);
	 free(stringOfFullFile);
	 stringOfFullFile = NULL;
    /*Closing files and freeing allocated memory*/
	close(iptr);
	remove(temp);
    close(fileThatWeRead);
	close(tempFileForSumFd);
...
/*Copied and edited from the course book*/
int msgwrite(void *buf, int len, int queueId) { /* output buffer of specified length */
	int error = 0;
	mymsg_t *mymsg;
	...
	if (msgsnd(queueId, mymsg, len, IPC_NOWAIT) == -1)
		error = errno;
	free(mymsg);
	if (error) {
		errno = error;
